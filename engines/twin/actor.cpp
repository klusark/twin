...
Actor::Actor(Common::SeekableReadStream *stream) :
		...
		_isFalling(false), _computeCollisionWithBricks(false), _doorMode(false), _canFall(false) {
...
Actor::Actor() : _entity(nullptr), _dest(nullptr), _dead(false), _facingActor(nullptr), _turning(false), _isMoving(false),
		...
		_heroYBeforeFall(0), _computeCollisionWithBricks(false), _doorMode(false), _canFall(false) {
...
void Actor::processCollision() {
	...
	if (_standingOn != nullptr) {
		_processPos += (_standingOn->_pos - _standingOn->_previousPos);
		_pos = _processPos;
		...
		if (brickShape) {
			...
			if (/*actor->staticFlags.bCanFall && */_standingOn == nullptr) {
				...
				if (brickShape) {
					...
					if (!_entity->_anim->getKeyframe()->_noFalling && _canFall) {
						_isFalling = true;
...
void Actor::checkCollisionWithActors() {
	Scene *s = g_twin->getCurrentScene();
	for (int i = 0; i < s->_numActors; ++i) {
		Actor *a = s->getActor(i);
		if (a == this || a == _standingOn/* || actor->staticFlags.bComputeLowCollision*/) {
			continue;
		}
